...
def arabic_classification():
    if request.method == "GET":
        ...
        if file and file_is_allowed(file.filename):
            name, predicted_class_label = get_image_prediction(file=file, model=arabic_model, encoder=arabic_encoder)
            
            return render_template("arabic_classification.html", show_result = True, predicted_class=predicted_class_label, edges_image=name)
...
def english_classification():
    if request.method == "GET":
        ...
        if file and file_is_allowed(file.filename):
            name, predicted_class_label = get_image_prediction(file=file, model=english_model, encoder=english_encoder)
            
            return render_template("english_classification.html", show_result = True, predicted_class=predicted_class_label, edges_image=name)
