...
def classification(language):
    alphabet = alphabets.get(language, alphabets['english'])  # Default to English if not found
    if language not in ['arabic', 'english']:
        return "Invalid language", 404
    
    if request.method == "GET":
        return render_template(f"classification.html", show_result=False, language=language, alphabet=alphabet)
    else:
...
def classification_name(language):
    alphabet = alphabets.get(language, alphabets['english'])  # Default to English if not found
    if language not in ['arabic', 'english']:
        return "Invalid language", 404
